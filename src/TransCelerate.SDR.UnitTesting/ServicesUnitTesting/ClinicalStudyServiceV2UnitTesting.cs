...
namespace TransCelerate.SDR.UnitTesting.ServicesUnitTesting
{
    public class ClinicalStudyServiceV2UnitTesting
    {
        ...
        public void GetPartialStudy_UnitTesting()
        {
            ...
            var method = ClinicalStudyService.GetPartialStudyElements("1", 0, user, Constants.StudyElementsV2);
            method.Wait();
            var result = method.Result;

            _mockClinicalStudyRepository.Setup(x => x.GetPartialStudyItemsAsync(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<string[]>()))
                 .Returns(Task.FromResult(null as StudyDefinitionsEntity));
            method = ClinicalStudyService.GetPartialStudyElements("1", 0, user, Constants.StudyElementsV2);
            method.Wait();
            result = method.Result;
        ...
        public void GetSOAV2_UnitTesting()
        {
            ...
            var method = ClinicalStudyService.GetSOAV2("1", studyEntity.Study.StudyDesigns[0].Id, studyEntity.Study.StudyDesigns[0].StudyScheduleTimelines[0].Id, 0, user);
            method.Wait();
            var result = method.Result;
            ...
            method = ClinicalStudyService.GetSOAV2("1", studyEntity.Study.StudyDesigns[0].Id, "", 0, user);
            method.Wait();
            result = method.Result;
            ...
            method = ClinicalStudyService.GetSOAV2("1", studyEntity.Study.StudyDesigns[0].Id, "Wf1", 0, user);
            method.Wait();
