...
namespace TransCelerate.SDR.UnitTesting
{
    public class HelperV3ClassesUnitTesting
    {
        ...
        public void DataFiltersUnitTesting()
        {
            ...
            Assert.IsNotNull(DataFiltersV3.GetProjectionForPartialStudyElements(Constants.ClinicalStudyElementsV3.Select(x => x.ToLower()).ToArray()));
        ...
        {
            HelperV3 helper = new();
            var stringArray = Constants.ClinicalStudyElementsV3.Where(x => x.StartsWith("s")).ToArray();

            Assert.IsNotNull(helper.RemoveStudyElements(stringArray, GetDtoDataFromStaticJson()));
            stringArray = Constants.ClinicalStudyElementsV3.Where(x => !x.StartsWith("s")).ToArray();
            Assert.IsNotNull(helper.RemoveStudyElements(stringArray, GetDtoDataFromStaticJson()));
        }
        [Test]
        ...
        {
            var studyDto = GetDtoDataFromStaticJson();
            Assert.IsTrue(UniquenessArrayValidator.ValidateArrayV3(studyDto.ClinicalStudy.StudyIdentifiers));
            studyDto.ClinicalStudy.StudyIdentifiers.Add(studyDto.ClinicalStudy.StudyIdentifiers[0]);
            Assert.IsFalse(UniquenessArrayValidator.ValidateArrayV3(studyDto.ClinicalStudy.StudyIdentifiers));
