        ...
        public async Task<IActionResult> GetSOAV5(string studyId, string studyDesignId, string scheduleTimelineId, int sdruploadversion)
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(studyId))
                {
                    ...
                    var SoA = await _studyService.GetSOAV5(studyId, studyDesignId, scheduleTimelineId, sdruploadversion, user).ConfigureAwait(false);
                    //var SoA = ;
            ...
            {
                _logger.LogInformation($"Ended Controller : {nameof(StudyV2Controller)}; Method : {nameof(GetSOAV5)};");
            }
        }
        ...
        public async Task<IActionResult> GeteCPTV5(string studyId, int sdruploadversion, string studydesignId)
        {
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(studyId))
                {
                    ...
                    var study = await _studyService.GeteCPTV5(studyId, sdruploadversion, studydesignId, user);
