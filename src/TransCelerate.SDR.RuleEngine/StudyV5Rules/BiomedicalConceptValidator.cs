    ...
    public class BiomedicalConceptValidator : AbstractValidator<BiomedicalConceptDto>
    {
        ...
        public BiomedicalConceptValidator(IHttpContextAccessor httpContextAccessor)
        {
			...
			RuleFor(x => x.Notes)
			  ...
			  .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
			  .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
			  .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(BiomedicalConceptValidator), nameof(BiomedicalConceptDto.Notes)), ApplyConditionTo.AllValidators);
		}
    }
}
