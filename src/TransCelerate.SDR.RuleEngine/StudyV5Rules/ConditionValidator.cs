    ...
    public class ConditionValidator : AbstractValidator<ConditionDto>
    {
        ...
        public ConditionValidator(IHttpContextAccessor httpContextAccessor)
        {
			...
			RuleFor(x => x.Notes)
			   ...
			   .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
			   .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
			   .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(ConditionValidator), nameof(ConditionDto.Notes)), ApplyConditionTo.AllValidators);
		}
    }
}
