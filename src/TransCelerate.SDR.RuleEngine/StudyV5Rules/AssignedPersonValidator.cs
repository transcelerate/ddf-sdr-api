    ...
    public class AssignedPersonValidator : AbstractValidator<AssignedPersonDto>
    {
        ...
        public AssignedPersonValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.PersonName)
                ...
                .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
                .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
                .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(AssignedPersonValidator), nameof(AssignedPersonDto.PersonName)), ApplyConditionTo.AllValidators)
                .SetValidator(new PersonNameValidator(_httpContextAccessor));
            ...
            RuleFor(x => x.OrganizationId)
                ...
                .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
                .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
                .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(AssignedPersonValidator), nameof(AssignedPersonDto.OrganizationId)), ApplyConditionTo.AllValidators);
        }
    }
}
