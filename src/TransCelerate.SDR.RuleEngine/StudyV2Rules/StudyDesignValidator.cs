    ...
    public class StudyDesignValidator : AbstractValidator<StudyDesignDto>
    {
        ...
        public StudyDesignValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.StudyScheduleTimelines)
                ...
                .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
                .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
                .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[Constants.UsdmVersion], nameof(StudyDesignValidator), nameof(StudyDesignDto.StudyScheduleTimelines)), ApplyConditionTo.AllValidators)
                .Must(x => UniquenessArrayValidator.ValidateArrayV2(x)).WithMessage(Constants.ValidationErrorMessage.UniquenessArrayError);
