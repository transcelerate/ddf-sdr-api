    ...
    public class BiomedicalConceptSurrogateValidator : AbstractValidator<BiomedicalConceptSurrogateDto>
    {
        ...
        public BiomedicalConceptSurrogateValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.BcSurrogateName)
               ...
               .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[Constants.UsdmVersion], nameof(BiomedicalConceptSurrogateValidator), nameof(BiomedicalConceptSurrogateDto.BcSurrogateName)), ApplyConditionTo.AllValidators);

            RuleFor(x => x.BcSurrogateDescription)
               .Cascade(CascadeMode.Stop)
               .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
               .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
               .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[Constants.UsdmVersion], nameof(BiomedicalConceptSurrogateValidator), nameof(BiomedicalConceptSurrogateDto.BcSurrogateDescription)), ApplyConditionTo.AllValidators);

            RuleFor(x => x.BcSurrogateReference)
               .Cascade(CascadeMode.Stop)
               .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
               .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
               .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[Constants.UsdmVersion], nameof(BiomedicalConceptSurrogateValidator), nameof(BiomedicalConceptSurrogateDto.BcSurrogateReference)), ApplyConditionTo.AllValidators);            
        }
    }
}
