    ...
    public class EligibilityCriterionValidator : AbstractValidator<EligibilityCriterionDto>
    {
        ...
        public EligibilityCriterionValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.Text)
               ...
               .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
               .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
               .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(EligibilityCriterionValidator), nameof(EligibilityCriterionDto.Text)), ApplyConditionTo.AllValidators);
