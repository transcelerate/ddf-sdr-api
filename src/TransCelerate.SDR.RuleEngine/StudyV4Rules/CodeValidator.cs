    ...
    public class CodeValidator : AbstractValidator<CodeDto>
    {
        ...
        public CodeValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.InstanceType)
                ...
                .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
                .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
                .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(CodeValidator), nameof(CodeDto.InstanceType)), ApplyConditionTo.AllValidators)
