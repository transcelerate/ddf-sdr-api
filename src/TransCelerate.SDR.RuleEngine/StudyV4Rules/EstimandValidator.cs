    ...
    public class EstimandValidator : AbstractValidator<EstimandDto>
    {
        ...
        public EstimandValidator(IHttpContextAccessor httpContextAccessor)
        {
            ...
            RuleFor(x => x.TreatmentId)
                ...
                .NotNull().WithMessage(Constants.ValidationErrorMessage.PropertyMissingError)
                .NotEmpty().WithMessage(Constants.ValidationErrorMessage.PropertyEmptyError)
               .When(x => RulesHelper.GetConformanceRules(_httpContextAccessor.HttpContext.Request.Headers[IdFieldPropertyName.Common.UsdmVersion], nameof(EstimandValidator), nameof(EstimandDto.TreatmentId)), ApplyConditionTo.AllValidators);
        }
    }
}
