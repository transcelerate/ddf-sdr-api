    ...
    public class CommonServices : ICommonService
    {
        ...
        public static List<SearchTitleResponseDto> SortStudyTitle(List<SearchTitleResponseDto> searchTitleDTOs, SearchTitleParametersDto searchParametersDTO)
        {
            if (!String.IsNullOrWhiteSpace(searchParametersDTO.SortBy))
            {
                ...
                {
                    "sponsorid" => searchParametersDTO.SortOrder == SortOrder.asc.ToString() ? searchTitleDTOs.OrderBy(s => s.Study.StudyIdentifiers != null ? s.Study.StudyIdentifiers.FindAll(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).Any() ? s.Study.StudyIdentifiers.Find(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).StudyIdentifierScope.OrganisationIdentifier ?? "" : "" : "").ToList()
                                                                                         : searchTitleDTOs.OrderByDescending(s => s.Study.StudyIdentifiers != null ? s.Study.StudyIdentifiers.FindAll(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).Any() ? s.Study.StudyIdentifiers.Find(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).StudyIdentifierScope.OrganisationIdentifier ?? "" : "" : "").ToList(),
        ...
        public async Task<object> SearchStudy(SearchParametersDto searchParametersDto)
        {
            try
            {
                ...
                searchResponseDtos = await AssignDynamicValuesAsync(searchParameters, searchResponseDtos, searchResponseEntities);
        ...
        public async Task<List<SearchResponseDto>> AssignDynamicValuesAsync(SearchParametersEntity searchParameters, List<SearchResponseDto> searchResponseDtos, List<SearchResponseEntity> searchResponseEntities)
        {
            searchParameters.UsdmVersion = Constants.USDMVersions.V4;
            var searchResponseV5 = await _commonRepository.SearchStudyV5(searchParameters);
        ...
        public static List<SearchResponseDto> SortSearchResults(List<SearchResponseDto> searchResponseDtos, SearchParametersDto searchParameters)
        {
            if (!String.IsNullOrWhiteSpace(searchParameters.Header))
            {
                return searchParameters.Header.ToLower() switch
                {
                    ...
                    "sponsorid" => searchParameters.Asc ? searchResponseDtos.OrderBy(s => s.Study.StudyIdentifiers != null ? s.Study.StudyIdentifiers.FindAll(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).Any() ? s.Study.StudyIdentifiers.Find(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).StudyIdentifierScope.OrganisationIdentifier ?? "" : "" : "").ToList()
                                                            : searchResponseDtos.OrderByDescending(s => s.Study.StudyIdentifiers != null ? s.Study.StudyIdentifiers.FindAll(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).Any() ? s.Study.StudyIdentifiers.Find(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope?.OrganisationType?.Decode)).StudyIdentifierScope.OrganisationIdentifier ?? "" : "" : "").ToList(),
