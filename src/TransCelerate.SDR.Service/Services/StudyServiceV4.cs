...
namespace TransCelerate.SDR.Services.Services
{
    public class StudyServiceV4 : IStudyServiceV4
    {
        ...
        /// <see langword="null"/> If no study is matching with studyId
        /// </returns>
        public async Task<object> GetSOAV4(string studyId, string studyDesignId, string scheduleTimelineId, int sdruploadversion)
        {
            try
            {
        ...
        /// <see langword="null"/> If no study is matching with studyId
        /// </returns>
        public async Task<object> GeteCPTV4(string studyId, int sdruploadversion, string studyDesignId)
        {
            try
            {
                ...
                studyDto.StudyDesigns.ForEach(design =>
                {
                    Core.DTO.eCPT.StudyDesignDto studyeCPTDto = new()
                    {
                        ...
                        ECPTData = new Core.DTO.eCPT.ECPTDataDto
                        {
                            ...
                            {
                                Acronym = studyDto.Titles.GetStudyTitle(Constants.StudyTitle.StudyAcronym),
                                AmendmentNumber = studyDto.Amendments.GetAmendmentNumber(),
                                ApprovalDate = studyDto.GetStudyProtocolVersionsV4(protocolVersions).GetProtocolEffectiveDate(Constants.DateValue.ProtocolEffectiveDate),
                                ...
                                SponsorName = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganizationType.Decode)).Select(x => x.StudyIdentifierScope.Name).FirstOrDefault(),
                                SponsorLegalAddress = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganizationType.Decode)).Select(x => x.StudyIdentifierScope.LegalAddress).FirstOrDefault() == null ? null
                                                                : studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganizationType.Decode)).Select(x => x.StudyIdentifierScope.LegalAddress).Select(x => $"{x.Line}, {x.City}, {x.District}, {x.State}, {x.PostalCode}, {x.Country?.Decode}").FirstOrDefault(),
                                ...
                                {
                                    ProtocolID = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganizationType.Decode)).Select(x => x.StudyIdentifier).FirstOrDefault(),
                                    ProtocolShortTitle = studyDto.Titles.GetStudyTitle(Constants.StudyTitle.BriefStudyTitle),
                                    ProtocolTitle = studyDto.Titles.GetStudyTitle(Constants.StudyTitle.OfficialStudyTitle)
                                }
                            },
                            ProtocolSummary = new Core.DTO.eCPT.ProtocolSummaryDto
                            {
                                ...
                                {
                                    NumberofParticipants = design.Population.GetNumberOfParticipantsV4(),
                            ...
                            {
                                VersionNumber = studyDto.GetStudyProtocolVersionsV4(protocolVersions) is not null ? studyDto.GetStudyProtocolVersionsV4(protocolVersions).ProtocolVersion : null,
                            },
