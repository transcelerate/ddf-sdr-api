...
namespace TransCelerate.SDR.Services.Services
{
    public class StudyServiceV3 : IStudyServiceV3
    {
        ...
        #region Constructor
        public StudyServiceV3(IStudyRepositoryV3 studyRepository, IMapper mapper, ILogHelper logger, IHelperV3 helper,
                    ...
                    Core.DTO.eCPT.StudyDesignDto studyeCPTDto = new()
                    {
                        ...
                        ECPTData = new Core.DTO.eCPT.ECPTDataDto
                        {
                            TitlePage = new Core.DTO.eCPT.TitlePageDto
                            {
                                ...
                                SponsorName = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganisationType.Decode)).Select(x => x.StudyIdentifierScope.OrganisationName).FirstOrDefault(),
                                SponsorLegalAddress = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganisationType.Decode)).Select(x => x.StudyIdentifierScope.OrganizationLegalAddress).FirstOrDefault() == null ? null
                                                                : studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganisationType.Decode)).Select(x => x.StudyIdentifierScope.OrganizationLegalAddress).Select(x => $"{x.Line}, {x.City}, {x.District}, {x.State}, {x.PostalCode}, {x.Country?.Decode}").FirstOrDefault(),
                                ...
                                {
                                    ProtocolID = studyDto.StudyIdentifiers.Where(x => CodeValueHelper.IsSponsorDecode(x.StudyIdentifierScope.OrganisationType.Decode)).Select(x => x.StudyIdentifier).FirstOrDefault(),
        ...
        public async Task<StudyDefinitionsDto> CreateNewVersionForAStudy(StudyDefinitionsEntity incomingStudyEntity, AuditTrailEntity existingAuditTrailEntity)
        {
            ...
            await _changeAuditService.ProcessChangeAudit(incomingStudyEntity.Study.StudyId, incomingStudyEntity.AuditTrail.SDRUploadVersion);
            return _mapper.Map<StudyDefinitionsDto>(incomingStudyEntity);
        }
        #endregion
